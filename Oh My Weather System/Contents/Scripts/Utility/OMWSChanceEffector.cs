...
{
    [Serializable]
    public class OMWSChanceEffector
    {
        ...
        public float GetChance(OMWSWeather weatherSphere)
        {
            switch (limitType)
            {
                case LimitType.Temperature:
                ...
                case (LimitType.AccumulatedSnow):
                    if (weatherSphere.GetModule<OMWSMaterialManager>())
                        return curve.Evaluate(weatherSphere.GetModule<OMWSMaterialManager>().m_SnowAmount);
                    else
                        return 0;
                case (LimitType.AccumulatedWetness):
                    if (weatherSphere.GetModule<OMWSMaterialManager>())
                        return curve.Evaluate(weatherSphere.GetModule<OMWSMaterialManager>().m_Wetness);
                    else
                        return 0;
                default:
                    return 1;
...
#if UNITY_EDITOR
    [CustomPropertyDrawer(typeof(OMWSChanceEffectorAttribute))]
    public class OMWSChanceEffectorDrawer : PropertyDrawer
    {
        OMWSChanceEffectorAttribute _attribute;

        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            _attribute = (OMWSChanceEffectorAttribute)attribute;
    ...
    public class OMWSChanceEffectorAttribute : PropertyAttribute
    {
        public OMWSChanceEffectorAttribute() { }
    }
}